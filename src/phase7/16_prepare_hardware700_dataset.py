...
def main():
    ...
    parser.add_argument("--test-per-class", type=int, default=350)
    parser.add_argument("--if-train-normal", type=int, default=50000)
    parser.add_argument("--pool-normal", type=int, default=12000)
    parser.add_argument("--pool-fraud", type=int, default=12000)
    ...
    dataset_path = out_data / "phase7_if_quantum_dataset_4q_hw700.npz"
    params_path = out_models / "phase7_if_plus_vqc_params_hw700.npz"
    ...
    print("IF train normal:", len(if_train))
    ...
    if len(normal_pool) < n_each or len(fraud_pool) < n_each:
        raise ValueError(f"Not enough rows. Need {n_each} per class.")
    ...
    final.to_csv(out_reports / "hardware700_dataset_source_rows.csv", index=False)

    counts = final.groupby(["split", "label"]).size().reset_index(name="count")
    counts.to_csv(out_reports / "hardware700_dataset_counts.csv", index=False)

    X_train = X[split == "train"]
    y_train = y[split == "train"]

    print("Training VQC params for HW700 dataset...")
    ...
    metadata = {
        ...
        "total_test_rows": int(2 * n_test),
        "note": "Separate larger hardware dataset for 700-sample IBM feasibility test.",
    }

    with open(out_reports / "hardware700_dataset_metadata.json", "w", encoding="utf-8") as f:
