...
def clean_model_name(name):
    ...
    if "kernel" in name:
        return "Isolation Forest + Quantum Kernel SVC IBM Hardware"

    if "vqc" in name:
        return "Isolation Forest + VQC IBM Hardware"
...
def main():
    ...
    perf = pd.read_csv(phase7 / "phase7_if_quantum_hardware_performance_metrics.csv")
    runtime = pd.read_csv(phase7 / "phase7_if_quantum_hardware_runtime_cost.csv")
    ...
    perf_final.to_csv(
        final / "isolation_forest_quantum_hybrid_hardware_performance_table.csv",
        index=False,
    )

    with open(final / "isolation_forest_quantum_hybrid_hardware_performance_table.txt", "w", encoding="utf-8") as f:
    ...
    runtime_final.to_csv(
        final / "isolation_forest_quantum_hybrid_hardware_runtime_cost_table.csv",
        index=False,
    )

    with open(final / "isolation_forest_quantum_hybrid_hardware_runtime_cost_table.txt", "w", encoding="utf-8") as f:
        f.write(runtime_final.to_string(index=False))

    print("Hardware performance table:")
    print(perf_final)

    print("\nHardware runtime/cost table:")
    print(runtime_final)
